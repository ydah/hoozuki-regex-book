...
RSpec.describe Hoozuki::Parser do
  describe '#parse' do
    ...
    it 'parses empty alternatives' do
      epsilon = Hoozuki::Node::Epsilon
      empty = Hoozuki::Parser.new('').parse
      middle_empty = Hoozuki::Parser.new('a||b').parse.children[1]
      empty_choices = Hoozuki::Parser.new('|').parse.children

      expect(empty).to be_a(epsilon)
      expect(middle_empty).to be_a(epsilon)
      expect(empty_choices).to all(be_a(epsilon))
    end
    ...
    context 'with grouping' do
      ...
      it 'parses empty group' do
        ast = Hoozuki::Parser.new('()').parse

        expect(ast).to be_a(Hoozuki::Node::Epsilon)
      end
    end
    ...
    context 'with repetition' do
      ...
      it 'parses one-or-more and optional repetition' do
        one_or_more = Hoozuki::Parser.new('a+').parse
        optional = Hoozuki::Parser.new('a?').parse

        expect(one_or_more.one_or_more?).to be true
        expect(optional.optional?).to be true
      end

      it 'rejects repeated quantifiers' do
        expect {
