...
RSpec.describe Hoozuki do
  describe '#match?' do
    ...
    context 'when choices have different lengths' do
      it 'tries the next choice after a partial match' do
        regex = Hoozuki.new('a|ab')

        expect(regex.match?('ab')).to be true
      end

      it 'can use a shorter choice after a longer choice' do
        regex = Hoozuki.new('ab|a')
    ...
    context 'with an empty choice' do
      it 'keeps the empty choice while trying the next choice' do
        regex = Hoozuki.new('|a')
    ...
    context 'with grouped patterns' do
      ...
      it 'matches empty patterns and empty alternatives' do
        expect(Hoozuki.new('').match?('')).to be true
        expect(Hoozuki.new('()').match?('')).to be true
        expect(Hoozuki.new('a|').match?('')).to be true
        expect(Hoozuki.new('a||b').match?('')).to be true
      end
    ...
    context 'with zero-or-more repetition' do
      ...
      context 'with an epsilon cycle' do
        it 'terminates while matching an empty string' do
          regex = Hoozuki.new('(|)*')
    ...
    context 'with combined quantifiers' do
      ...
      it 'matches various combinations' do
        ...
        %w[()+ (a*)+ (a?)+ (a*)*].each do |pattern|
          expect(Hoozuki.new(pattern).match?('')).to be true
        end
      end
    end
  end
